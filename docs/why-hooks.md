...
훅이 잘못 차단하는 경우(오탐) 일시적으로 우회할 수 있습니다.

```bash
git commit --no-verify -m "메시지"   # 커밋 훅 우회
git push --no-verify                  # 푸시 훅 우회
```

> ⚠️ `--no-verify`는 **모든 보안 검사를 건너뜁니다**.
> 사용 후 반드시 오탐 원인을 점검하고 훅 패턴을 수정하세요.
